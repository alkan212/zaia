...
export async function logUser(res, db, email) {
    ...
    await res.setHeader('Set-Cookie', [
        serialize('token', user.token, { path: '/', maxAge: expiresIn }),
...
export function formatCurrency(number, ifDecimal = true) {
    ...
    return `${formattedIntegerPart}${ifDecimal == false && decimalPart !== "00" ? `,${decimalPart}` : ""}`;
}
...
export function shouldTextBeWhite(hexColor) {
    try{
    // Make sure the color string has a '#' at the start
    if (hexColor[0] !== "#") {
        hexColor = "#" + hexColor;
      }
    
      // Calculate brightness (YIQ Formula)
      var bgColor = hexColor.slice(1); // Remove '#' from color code
      ...
      //Return true if text should be white, else return false
      return Y < 128;
    }catch(e){
        console.log(e)
        return false
...
export function resizeImage(src, maxWidth, maxHeight, callback) {
    const img = new Image();
    img.src = src;
